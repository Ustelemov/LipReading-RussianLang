...
## Соеденить два txt-файла в один (нужно для файлов фонем, когда видео разбивается на два)
```
python stretch_two_txts.py \
--f '/content/phonemes_frames.txt' \
--s '/content/phonemes_frames1.txt' \
--o '/content/out.txt'
```
- --f - путь к первому txt-файлу (в выходном будет также идти первым), из которого будут браться данные
- --s - путь ко второму txt-файлу, из которого будут браться данные
